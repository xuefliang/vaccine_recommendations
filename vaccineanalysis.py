...
@pd.api.extensions.register_dataframe_accessor("vaccineanalysis")
class VaccineAnalysisAccessor:
    ...
    def calculate_vaccine_rate(self, by_age=False):
...
person_vaccination = pd.read_csv('/mnt/d/标准库接种率/标准库数据/person_standard_vaccination.csv', dtype={'VACCINATION_CODE': str}).clean_names()

person2 = person.merge(person_vaccination, how='left', left_on='id', right_on='person_id')
...
person2.vaccineanalysis.calculate_vaccine_rate(by_age=True).to_excel('/mnt/d/标准库接种率/vacc_rate.xlsx', index=False)