...
class VaccinationAnalyzer:
    ...
    def pivot_data(self):
        # 先进行pivot操作
        self.person = self.person.pivot_wider(
            names_from=['dose', 'vaccine_name'], 
            values_from=['vaccination_date'], 
            values_agg="first"
        )
        ...
        agg_exprs = {}
        for col in self.expected_columns:
            agg_exprs[col] = ibis.coalesce(self.person[col], ibis.null().cast(datatypes.date)).min().name(col)
        
        # 添加其他需要保留的列
        keep_columns = ['patient_name', 'gender_code', 'birth_date', 'current_management_code', 'age_month']
        for col in keep_columns:
            agg_exprs[col] = self.person[col].first().name(col)
        
        # 按id_x分组并聚合
        self.person = self.person.group_by('id_x').aggregate(**agg_exprs)
    ...
    def substitute_vaccines(self):
        self.person = self.person.mutate(
            ...
            # 乙脑疫苗替换
            temp_乙脑疫苗_1=ibis.coalesce(_.乙脑疫苗_1, _.乙脑灭活疫苗_2),
            
            temp_乙脑疫苗_2=ibis.case()
                .when((_.vaccination_month <= 72),
