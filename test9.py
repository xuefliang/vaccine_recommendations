...
class VaccinationAnalyzer:
    ...
    def process_vaccine(self):
        ...
        vaccine_aggs = {
            vaccine: ibis.coalesce(self.person[vaccine], ibis.null().cast(datatypes.date)).max().name(vaccine)
    ...
    def get_coverage(self):
        recommendations_df = self.analyze().to_pandas().assign(age=lambda x: x.age_month)
...
person = (
    ...
    # .filter(_.id_x=='618eeeaedae4474993071697b9e7fef1')
)
...
recommendation = analyzer.coverage().filter(_.current_management_code=='334878619388').to_pandas()

# combined_result2 = analyzer.get_coverage().query("current_management_code=='334878619388'")
