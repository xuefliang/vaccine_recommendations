...
id_1=jzjc.query('vaccine_name == "乙脑疫苗" & jzjc == 1')['id_x']
jev2_ying0=(
    jzjl
    .query("id_x in @id_1 & age <=6 & age>=2 & vaccine_name=='乙脑疫苗' & year_month<@mon_stat & jc==1")
    .assign(
        vaccination_date_plus_interval=lambda df: df['vaccination_date'] + pd.DateOffset(months=12),
        vaccination_date_plus_13_months=lambda df: df['vaccination_date'] + pd.DateOffset(months=13+12)
    )
...
# 不符合要求的2剂次接种
id_2=jzjl.query("vaccine_name == '乙脑疫苗' & jc == 2 & year_month > @mon_stat & age <=6 & age>=2")['id_x']
...
result=jev2_ying0.merge(jev2_ying1, on='current_management_code', how='outer')
result=result.merge(jev2_shi, left_on='current_management_code',right_on='vaccination_org', how='outer')
