...
id_1=jzjc.query('vaccine_name == "百白破疫苗" & jzjc == 3')['id_x']
dpt4_ying0=(
    jzjl
    .query("id_x in @id_1 & age < 6 & months>=18 & vaccine_name=='百白破疫苗' & year_month<@mon_stat & jc == 3")
    .assign(
        vaccination_date_plus_interval=lambda df: df['vaccination_date'] + pd.DateOffset(months=6),
        vaccination_date_plus_13_months=lambda df: df['birth_date'] + pd.DateOffset(months=13+18),
    )
...
# 不符合要求的4剂次接种，4岁应种
id_2=jzjl.query("vaccine_name == '百白破疫苗' & jc == 4 & year_month > @mon_stat & age < 6 & months>=18")['id_x']
...
dpt4_ying1=(
    ...
    .assign(
        vaccination_date_plus_interval=lambda df: df['vaccination_date'] + pd.DateOffset(months=6),
        vaccination_date_plus_13_months=lambda df: df['vaccination_date'] + pd.DateOffset(months=13+6),
    )
    .loc[lambda df: (df['vaccination_date_plus_interval'] <= mon_end) & (df['vaccination_date_plus_13_months'] > mon_end)]
    .groupby(['current_management_code'])
...
result=dpt4_ying0.merge(dpt4_ying1, on='current_management_code', how='outer')
result=result.merge(dpt4_shi, left_on='current_management_code',right_on='vaccination_org', how='outer')
