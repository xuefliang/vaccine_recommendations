...
id_1=jzjc.query('vaccine_name == "含麻疹成分疫苗" & jzjc == 1')['id_x']
mcv2_ying0=(
    jzjl
    ...
    .assign(
        ...
        vaccination_birth_date=lambda df: df['birth_date'] + pd.DateOffset(months=18+13)
    )
    .loc[lambda df: (df['vaccination_date_plus_interval'] <= mon_end) & ((df['vaccination_date_plus_13_months'] > mon_end)  | (df['vaccination_birth_date']>mon_end))]
...
# 不符合要求的2剂次接种
id_2=jzjl.query("vaccine_name == '含麻疹成分疫苗' & jc == 2 & year_month > @mon_stat & age <=6 & months>=18")['id_x']
...
result=mcv2_ying0.merge(mcv2_ying1, on='current_management_code', how='outer')
result=result.merge(mcv2_shi, left_on='current_management_code',right_on='vaccination_org', how='outer')
