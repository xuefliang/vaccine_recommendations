...
id_1=jzjc.query('vaccine_name == "百白破疫苗" & jzjc == 0')['id_x']
dpt1_ying0=(
    person2
    .query("id_x in @id_1 & age <= 5 & months>=3")
    .assign(
        vaccination_date_plus_13_months=lambda df: df['birth_date'] + pd.DateOffset(months=13+3)
    )
...
# 不符合要求的1剂次接种
id_2=jzjl.query("vaccine_name == '百白破疫苗' & jc == 1 & year_month > @mon_stat & age <= 5 & months>=3")['id_x']
...
result=dpt1_ying0.merge(dpt1_ying1, on='current_management_code', how='outer')
result=result.merge(dpt1_shi, left_on='current_management_code',right_on='vaccination_org', how='outer')
