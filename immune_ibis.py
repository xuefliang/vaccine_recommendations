...
class Immune:
    def __init__(self, df: ibis.expr.types.Table, current_date: ibis.expr.types.TimestampScalar):
        ...
        self.vaccine_code_map = {
            ...
            '0401': '百白破疫苗', '0402': '百白破疫苗', '0403': '百白破疫苗', '4901': '百白破疫',
            '1201': '麻疹成分疫苗', '1301': '含麻疹成分疫苗', '1401': '含麻疹成分疫苗',
    ...
    def months_between(self, start, end):
        ...
        end_year = end.execute().year
        end_month = end.execute().month
    ...
    def get_recommended_dates(self, vaccine_name: str, birth_date: ibis.expr.types.TimestampValue, vaccinations: ibis.expr.types.ColumnExpr) -> ibis.expr.types.ColumnExpr:
        ...
        min_intervals = vaccine_info["min_interval"]
        if callable(min_intervals):
            min_intervals = min_intervals(birth_date)

        schedule = vaccine_info["schedule"]

        recommended_dates = []
        for i in range(vaccine_info["doses"]):
            if i == 0:
                recommended_date = ibis.case()\
                    .when(vaccinations.isnull().any() | 
                          ((vaccinations - birth_date).days() / 30 < min_intervals[i]) | 
                          (vaccinations.max() > self.current_date),
                          birth_date + ibis.interval(months=schedule[i]))\
                    .else_(ibis.NA)
            else:
                previous_vaccination = vaccinations.sort().nth(i-1)
                current_vaccination = vaccinations.sort().nth(i)
                recommended_date = ibis.case()\
                    .when(current_vaccination.notnull() & (current_vaccination < self.current_date), ibis.NA)\
                    .when(previous_vaccination.notnull(),
                          ibis.greatest(previous_vaccination + ibis.interval(months=min_intervals[i]), 
                                        birth_date + ibis.interval(months=schedule[i])))\
                    .else_(ibis.NA)
    ...
    def _generate_initial_recommendations(self) -> ibis.expr.types.Table:
        ...
        aggregated_data = aggregated_data.mutate(
            current_age_months=(
                (self.current_date.year() - aggregated_data.birth_date.year()) * 12 +
        ...
        for row in aggregated_data.itertuples():
            ...
            for vaccine_name, vaccine_info in self.vaccines.items():
                ...
                vaccinations = vaccine_data.order_by('vaccination_date').vaccination_date
    ...
    def other_vaccines(self) -> ibis.expr.types.Table:
        ...
        for (id_x, vaccine_name), group in other_vaccines.group_by(['id_x', 'vaccine_name']).iterate():
            ...
            current_age_months = (
                (self.current_date.year() - birth_date.year()) * 12 +
                (self.current_date.month() - birth_date.month())
            ).cast('int32')
            latest_management_code = group.current_management_code.last()
            vaccine_entry_org = group.entry_org.first()

            # 按vaccination_date排序
            sorted_group = group.order_by('vaccination_date')

            for i, row in enumerate(sorted_group.iterate(), start=1):
                vaccination_date = row.vaccination_date
                vacc_month = (
                    (vaccination_date.year() - birth_date.year()) * 12 +
                    (vaccination_date.month() - birth_date.month())
                ).cast('int32')
    ...
    def process_vaccines(self):
        def process_group(group):
            # 处理流脑疫苗第一种情况
            condition1 = (group.vaccine_name == '流替代疫苗') & (group.dose == 1) & (group.vacc_month < 6)
