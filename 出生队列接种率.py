...
def cohort_vaccination_coverage(person, management_code=None, age=None):
    ...
    if management_code is not None:
        filtered_person = filtered_person.filter(pl.col('current_management_code') == management_code)
    ...
    result = (
        ...
        .join(
            filtered_person
            .group_by(['current_management_code', 'age', 'vaccine_name', 'vaccination_seq'])
            .agg(pl.col('id_x').n_unique().alias('接种数')),
            on=['current_management_code', 'age'],
            how='left'
        )
        .with_columns(
            (pl.col('接种数') / pl.col('应种数') * 100)
            .round(2)
            .alias('接种率')
        )
